...
# Наворотил классы.
# Первый класс rel сделал как словарь, но просто синтаксис через точку как-то более приятен. Не знаю правильно ли это.
# В классах сделал атрибуты класса чтобы можно было изолированно хранить данные общие для всех классов этого типа и
# доступаться к ним от всех объектов этого класса. Правильно ли это? Или как-то по другому надо было реализовать
# Хотелось все сделать объекто-ориентировано, но на мой неискушенный взгляд все это как-то ужасно выглядит.
# В процессе программирования в очередной раз понял, что совершенно не понятно как проектировать классы.
#  Да, и в принципе как проектирвать программу, # состоящую чуть больше, чем из одной функции.
# Это знание у меня полностью отсутствует, а потребность в нем всплывает сразу же как только
# плюс-минус узнаешь синтаксис языка. Как в этом разобраться? Что читать конкретно? Макконела "Совершенный код"?
# Или что-то есть более локаничное. Что посмотреть? Может курс какой-то?

# И хотелось бы посмотреть на парочку реализаций этой задачи, которые с вашей точки зрения удачны.
##
